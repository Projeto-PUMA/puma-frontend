...
class ProjectSubmission extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { value: '', showJuridic: false, cep: '', uf: '', localidade: '', bairro: '', logradouro: '', id: -1, cnpj: '', razao: '', nomeEmpresa: '' };
    ...
    this.changeRazao = this.changeRazao.bind(this);
    this.changeNomeEmpresa = this.changeNomeEmpresa.bind(this);
  }
  ...
  changeNomeEmpresa(event) {
    this.setState({ nomeEmpresa: event.target.value });
  }

  changeRazao(event) {
    this.setState({ razao: event.target.value });
  }

  searchCnpj(cnpj) {
    console.log(cnpj)
    CnpjPromise(cnpj).then(data => {
      this.setState({nomeEmpresa: data.fantasia, razao: data.nome })
      console.log(data);
    }).catch(err => {
      // console.log(err);
    })
      ...
      return (
        <div>
          <FormGroup>
            ...
            <Input
              ...
              required
            />
            {/* <div style={{ marginTop: -12, fontSize: 12, color: 'red' }}>CNPJ Inválido</div> */}
            <button onClick={this.searchCnpj((this.state.cnpj).replace(/\D+/g, ''))} disabled>Pesquisar</button>
          </FormGroup>
        <FormGroup>
        ...
        <FormGroup>
          ...
          <Input
            ...
            // onChange={this.changeNomeEmpresa}
            required
            disabled
          />
        </FormGroup>
